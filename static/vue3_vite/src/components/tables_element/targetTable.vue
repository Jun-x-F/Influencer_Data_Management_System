<template>
  <div class="form-column">
    <el-table
      ...
      :default-sort="{ prop: 'parentId', order: 'descending' }"
    ...
    >
      ...
      >
        ...
        <template v-if="column.isLink" #default="scope">
          <a
            v-if="column.prop === '视频链接' && isValidURL(scope.row[column.prop])"
          ...
          <a
            v-else-if="column.prop === '红人名称' && isValidURL(scope.row['视频链接'])"
            :href="scope.row['视频链接']"
            target="_blank"
            rel="noopener noreferrer"
          >
          ...
          <a
            v-else-if="column.prop === '物流进度' && isValidURL(scope.row['物流单号'])"
            :href="scope.row['物流单号']"
            target="_blank"
            rel="noopener noreferrer"
          >
...
const fixedColumns: Column[] = [
  ...
  { prop: "平台", label: "平台", width: "auto", merge: true },
...
const filterTableData = computed(() => {
  if (props.changeId || props.changeId !== null) {
    return tableData.value.filter(data => data.parentId === props.changeId);
  } else if (props.changeProduct || props.changeProduct !== null) {
    return tableData.value.filter(data => props.changeProduct.has(data.产品));
  } 
...
onMounted(async () => {
  try {
    ...
    console.log("Merge Data:", mergeData.value["parentId"]);
  } catch (error) {
